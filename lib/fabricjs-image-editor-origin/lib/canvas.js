...
(function () {
    ...
    var canvas = function () {
        try {
            ...
            const fabricCanvas = new fabric.Canvas('c').setDimensions({ width: 512, height: 512 });
            ...
            window.setAnnotationEditorJson = (json) => {
                fabricCanvas.loadFromJSON(json, fabricCanvas.renderAll.bind(fabricCanvas));
                window.lastSavedJson = json;
            }

            window.restoreLastSaved = () => {
                fabricCanvas.loadFromJSON(window.lastSavedJson, fabricCanvas.renderAll.bind(fabricCanvas));
            }
