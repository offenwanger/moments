...
describe('Test Moment Panel', function () {
    ...
    describe('edit tests', function () {
        it('should update model name', async function () {
            ...
            expect(TestUtils.model().moments[0].model3Ds[0].name).toBe('test.glb');
            ...
            expect(TestUtils.model().moments[0].model3Ds[0].name).toBe("new name");
        });

        it('should update x value', async function () {
            await TestUtils.createAndOpenModel3D();
            expect(TestUtils.getInputValue("#model3D-position-x-input")).toBe(0);
            expect(TestUtils.model().moments[0].model3Ds[0].x).toBe(0);
            ...
            expect(TestUtils.model().moments[0].model3Ds[0].x).toBe(10);
        });
    });
});